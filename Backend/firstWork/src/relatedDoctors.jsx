...
const RelatedDoctors = ({speciality, docId}) => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
      <h1 className='text-3xl text-center font-medium'>Related Doctors to Book</h1>
        <p className='sm:w-1/3 text-center text-sm'>Simply brows through our extensive list of trusted doctors</p>
        <div className='w-68 lg:w-full sm:w-full grid sm:grid-cols-3 md:grid-rows-2  gap-4 pt-6 gap-y-6 px-3 sm:px-0'>
            ...
            <div onClick={()=>{navigate(`/appointment/${item._id}`); scrollTo(0,0)}} className='border border-blue-200 rounded-x1 overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'  key={index}>
                ...
                <div className='p-4 '> 
                    <div className='flex items-center gap-2 text-sm text-center text-green-500 '>
                        <p className='w-2 h-2 bg-green-500 gap-43 rounded-full'></p> <p>Available</p>
                    </div>
